...
- The transcript may be formatted as "SPEAKER_00: ...\nSPEAKER_01: ..." when multiple distinct voices were detected — treat each distinct SPEAKER_NN label as a genuinely different person, and write the overview/key_points accordingly ("the group discussed...", "two people discussed...") rather than defaulting to "the speaker" (singular) as if it were one person talking, when the transcript clearly shows otherwise. If there's no such labeling, treat it as a single continuous voice as before.
- SPEAKER_00/SPEAKER_01/etc. are NOT real names — never use them as a participant's name. Only add someone to participants when an actual name is stated in the transcript itself (e.g. someone is addressed by name, or introduces themselves). A multi-speaker conversation where nobody is ever named should still have an empty participants list — the speaker labels tell you multiple people were present, not who they are.
